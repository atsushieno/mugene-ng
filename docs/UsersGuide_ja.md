（このドキュメントは、大部分は書き上がっていますが、まだ説明していない部分がいくつかあります。またmugene-ngで新しく拡張されたMIDI 2.0サポートなどについては、英語版ドキュメントのみで更新されているのでそちらを参照して下さい。）
...
[identifier]の名前と[value]の値で変数を設定します。設定された変数は mml_operator_dollar 演算子 $ によって参照できます。
